...
GtkWidget *open_item;
GtkWidget *dialog1;
// Загрузка пользовательского значка
//GdkPixbuf *icon_pixbuf = gdk_pixbuf_new_from_file("app_retometr.ico", NULL);
...
// void cancelLoading3(GtkWidget *widget, gpointer user_data) {
//     gtk_widget_set_sensitive(GTK_WIDGET(window1), TRUE);
...
//         int fileCount = 0;
//         double progress = 0.0; // Инициализируем прогресс
...
//     // Установка пользовательского значка
//     //gtk_window_set_icon(GTK_WINDOW(window2), icon_pixbuf);
...
//     // Установка пользовательского значка
//     //gtk_window_set_icon(GTK_WINDOW(window_termins), icon_pixbuf);
...
int main(int argc, char *argv[]) {
    ...
    // Установка пользовательского значка
    //gtk_window_set_icon(GTK_WINDOW(window1), icon_pixbuf);
    //gtk_window_set_icon(GTK_WINDOW(window_bar), icon_pixbuf);
