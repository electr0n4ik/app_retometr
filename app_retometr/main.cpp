...
void startLoading() {
    ...
    if (dnd_flag)
    {
        ...
        folder_path = gtk_file_chooser_get_filename(chooser);
		#ifdef _WIN32
            boost_path = gtk_file_chooser_get_filename(chooser);
            ...
            // std::cout << path << " <- not dnd" << std::endl;
		#elif __linux__
			//const gchar *folder_path_decode = gtk_file_chooser_get_filename(chooser);
            folder_path = gtk_file_chooser_get_filename(chooser);
			// folder_path = boost::filesystem::u8path(folder_path_decode);
            std::cout << folder_path << " test" << std::endl;
            //boost::filesystem::path folder_path = boost::filesystem::path(folder_path_decode).string();

		#endif
		gtk_widget_destroy(dialog1);
    } 

    // std::cout << newString << " <- test1" << std::endl;
    // boost::filesystem::path folder_path(newString);
    std::string utf8_path = boost_path.string();


    std::cout << utf8_path << " <- test1" << std::endl;
    std::cout << folder_path << " <- test2" << std::endl;

	if (boost::filesystem::is_directory(utf8_path)) 
    {
        std::cout << folder_path << " <- test3" << std::endl;
    ...
    } else 
    {
        ...
        //     gtk_widget_destroy(dialog1);
        // }
        std::cout << "3return" <<" путь не найден или ошибка" << std::endl;
        totalFiles = 0;
        fileCount = 0;
        number = 0;
        folder_path = "";
	...
	totalFiles = std::distance(boost::filesystem::directory_iterator(folder_path), boost::filesystem::directory_iterator{});
    ...
    for (const auto &entry: boost::filesystem::directory_iterator(folder_path)) {
        ...
        boost::filesystem::path file_path = entry.path().string();
        ...
        if (k == 0) {
                ...
                std::cout << " " << start_reg << " start_reg" << std::endl;
				std::string formattedTimeStart = formatted_datetime(paramAttrInBlock[0]);
                std::cout << " " << formattedTimeStart << " formattedTimeStart" << std::endl;
...
static void on_drag_data_received(GtkWidget *widget, GdkDragContext *context, gint x, gint y, GtkSelectionData *data, guint info, guint time, gpointer user_data) {
    ...
    if (const guchar *selection_data_bytes = gtk_selection_data_get_data(data)) {
        ...
        std::string output = RemoveNewlines(newString);
            ...
            std::cout << "folder_path exists: " << boost::filesystem::exists(newString) << std::endl;
            std::cout << "folder_path is_directory: " << boost::filesystem::is_directory(newString) << std::endl;
