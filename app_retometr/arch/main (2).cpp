...
void startLoading() {
	...
	if (!std::filesystem::is_directory(folder_path)) {
		...
		for (const auto &entry : std::filesystem::directory_iterator(folder_path)) {
		    if (entry.is_directory()) {
				...
				if (entry.path().extension() == ".pke" || entry.path().extension() == ".xml") {
    ...
    for (const auto &entry: std::filesystem::directory_iterator(folder_path)) {
		...
		if (number == 1) {
			...
			liststoreResult = gtk_list_store_new(32, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
								                 G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
								                 G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
								                 G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
								                 G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
         ...
         if (k == 1){
            ...
            for (int i = 0; i < numColumns; ++i) {
				if (i <= numColumns){
		            ...
		            //std::cout << g_strdup_printf("%s", titles[i]);
		...
		if (k == 2){
			...
			for (pugi::xml_node resultNode = root.child("Result_Check_PKE"); resultNode; resultNode = resultNode.next_sibling("Result_Check_PKE")) {
				...
				//printf("TimeTek: %s\n", resultNode.attribute("TimeTek").value());
				
				gchar* values[numColumns] = { nullptr }; // Инициализация массива нулевыми указателями
    ...
    // gtk_list_store_clear(liststoreResult);
    std::cout << " Конец функции startLoading " << std::endl;
    k = 0;
